...
export async function POST() {
  try {
    ...
    const results = { rentals: 0, purchases: 0, prestations: 0, errors: 0, skipped: 0 }

    // ── LOCATIONS ──────────────────────────────────────────────────────────────
    const { data: rentalReservations } = await supabase
      .from('rental_reservations')
      .select(`
        id, google_event_id, reservation_status,
        delivery_address,
    ...
    return NextResponse.json({
      success: true,
      message: `Backfill terminé : ${results.rentals} locations, ${results.purchases} achats, ${results.prestations} prestations créés. ${results.skipped} ignorés (sans dates). ${results.errors} erreurs.`,
      results,
    })
  } catch (error) {
